...
def get_data(final_features):
   ...
   if (pred<0.5):
        pred='You have '+ str((1-pred)*100)+'%'+' possibility of having heart disease.Please take immediate action to cure your self.'
   elif (0.5<=pred<0.75):
        pred='You have '+ str((1-pred)*100)+'%'+' possibility of having heart disease.You have to be careful and take some actions.'
   elif (pred>0.75):
        pred='You have '+ str((1-pred)*100)+'%'+' possibility of having heart disease.You are safe'
 
    
   return {
       "fulfillmentText" : pred,
       "intent": final_features
